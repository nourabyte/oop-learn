...
2. `Protected`: Protected members of a superclass are inherited by its subclasses and remain protected in the subclass. They are accessible within the same package and by subclasses, even if they are in a different package.
...
4. `Private` : Private members of a superclass are not inherited by its subclasses and are not accessible in the subclass.
